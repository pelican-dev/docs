...
* `sh: 1: mysql: not found` if you are using Mariadb ^11.4 you'll have to restore the mysql alias dy doing `cd /usr/bin;ln -s mariadb mysql` (You need mariadb-client installed on the panel machine eventhough your mariadb-server is hosted elsewhere)
