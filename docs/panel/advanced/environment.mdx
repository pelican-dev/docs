...
Panel settings are stored in environment variables within your `.env` file. In standard installations, you do not need to manually modify the `.env` file; these options will be set by artisan, the web installer, and the settings menu. For automated installations, it may be useful to set them programmatically. 
