...
### Other
These settings are not configured by the installer but can be set in the admin settings area, or using these environment variables.

- `MAIL_MAILER`: The driver to use for sending email. `[log, smtp, mailgun, sendmail]` Default: log
...
- `MAIL_ENCRYPTION`: What transport security type to use for the SMTP server. `[log, sendmail, smtp, mailgun, postmark, mandrill]`
