...
Run `php artisan p:plugin:make` to create the basic files and folders for your new plugin. This will create the `plugin.json`, the main plugin file, a config file and a service provider.
...
<Admonition type="warning" title="Subuser Permissions">
    This will only work for admin role permissions, not subuser permissions! There is currently no way to register custom subuser permissions.
</Admonition>
