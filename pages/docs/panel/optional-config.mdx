...
By default, Pelican uses local storage via daemon for backups. That said, this method of backup storage can be explicitly set with the following configuration in the `.env` file:

```sh copy
APP_BACKUP_DRIVER=daemon
```

Do note that, when using local storage via the daemon, the destination for backups is set in the daemon config file with the following setting key:

```yml copy {2} /path/to/backup/storage/
system:
