...
|------------------|---------|:------------------:|-------------------------------------------------------------|
| **Ubuntu**       | 20.04   |         ✅︎         | Documentation written assuming Ubuntu 20.04 as the base OS. |
|                  | 22.04   |         ✅︎         |     MariaDB can be installed without the repo setup script. |
|                  | 24.04   |         ✅︎         |                                                             |
| **Rocky Linux**  | 9       |         ✅︎         |                                                             |
| **Debian**       | 11      |         ✅︎         |                                                             |
|                  | 12      |         ✅︎         |                                                             |
...
* PHP `8.2` with the following extensions: `cli`, `openssl`, `gd`, `mysql`, `PDO`, `mbstring`, `tokenizer`, `bcmath`, `xml` or `dom`, `curl`, `zip`,`intl` and `fpm`
...
See below to create a user and database for your panel quickly.

```sql {2} /yourPassword/
mysql -u root -p
CREATE USER 'pelican'@'127.0.0.1' IDENTIFIED BY 'yourPassword';
...
<Tabs items={['NGINX/Apache','Rocky Linux Apache','Rocky Linux NGINX']}>
    <Tabs.Tab>
    ```sh copy
...
```text filename="/etc/systemd/system/pelicanq.service" copy
# Pelican Queue File
# ----------------------------------
